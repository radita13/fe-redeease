...
export default function FleetHeader({ searchTerm, setSearchTerm, onRefresh, onOpenAdd }) {
    ...
    <header className='bg-surface border-outline-variant/30 sticky top-0 z-40 flex min-h-20 shrink-0 flex-col items-start justify-between gap-4 border-b px-6 py-4 md:h-20 md:flex-row md:items-center md:px-8 md:py-0'>
      <div className='w-full md:w-auto'>
        ...
        <p className='text-on-surface-variant mt-1 text-xs font-semibold'>
          Manage vehicle inventory and operational dispatch status
        </p>
      </div>

      <div className='flex w-full items-center gap-4 px-0 md:max-w-lg md:flex-1 md:px-8'>
        <div className='group relative flex-1'>
          ...
          <Input
            ...
            placeholder='Search by plate no, driver, type...'
          />
        </div>
      </div>
      ...
      <div className='flex w-full items-center justify-end gap-3 md:w-auto md:justify-start'>
        ...
        <div className='bg-outline-variant w-1px h-6'></div>
        <Button
          onClick={onOpenAdd}
          className='bg-primary text-on-primary hover:bg-primary/95 hover:shadow-primary/25 text-label-lg font-label-lg h-11 shrink-0 cursor-pointer items-center gap-2 rounded-full border-none px-6 font-bold transition-all duration-150 hover:shadow-lg active:scale-95'
        >
