...
export default function UsersHeader({ searchTerm, setSearchTerm, onRefresh, onExport }) {
    ...
    <header className='bg-surface border-outline-variant/30 sticky top-0 z-40 flex min-h-20 shrink-0 flex-col items-start justify-between gap-4 border-b px-6 py-4 md:h-20 md:flex-row md:items-center md:px-8 md:py-0'>
      <div className='w-full md:w-auto'>
        ...
        <p className='text-on-surface-variant mt-1 text-xs font-semibold'>
          View, filter, suspend, and manage all user and admin accounts
        </p>
      </div>

      <div className='flex w-full flex-col gap-4 px-0 sm:flex-row sm:items-center md:max-w-2xl md:flex-1 md:px-8'>
      ...
      <div className='flex w-full items-center justify-end gap-3 md:w-auto md:justify-start'>
        ...
        <button
          onClick={onExport}
          className='border-primary text-primary hover:bg-primary/5 hover:text-primary text-label-lg font-label-lg flex h-11 shrink-0 cursor-pointer items-center gap-2 rounded-full border-2 px-6 font-bold transition-all duration-150 active:scale-95'
        >
